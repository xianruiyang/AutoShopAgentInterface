...
- `scripts/autoshop-agent.exe`: Packaged CLI binary, currently `v0.8.130`.
- `references/`: Detailed command, format, UI refresh, testing, EtherCAT slave template references, motion-axis `outputDevice` JSON binding, and CANLink `programConfig` D/M editing.
...
- `references/AutoShopWorkspaceJsonReference.md`: Split workspace JSON reference for POU, variables, interrupts, H5U modules, EtherCAT, motion axes, EtherNet/IP, and CAN(CANLink).
