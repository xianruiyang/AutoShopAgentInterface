...
- CANopen: dynamic `配置/CAN(CANopen)/_node.config.json`, read-only `canOpen.catalog`, writable existing `canOpen.dataConfig.objectTable[]` values, `slaves[].general.producerHeartbeatTimeMs`, and existing `rxPdos[]`/`txPdos[]` summary values parsed from `canopen.data`, read-only calibrated `canOpen.dataConfig.ioMappings[]`, plus raw `canopen.data` / `canopen.up` preservation when those files exist
