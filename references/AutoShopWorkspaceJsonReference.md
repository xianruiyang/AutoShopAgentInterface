...
- CANopen: dynamic `配置/CAN(CANopen)/_node.config.json`, read-only `canOpen.catalog`, writable existing `canOpen.dataConfig.objectTable[]` values, `slaves[].general.producerHeartbeatTimeMs`, existing `rxPdos[]`/`txPdos[]` summary values, existing `ioMappings[].startRegister` D start registers parsed from `canopen.data`, read-only empty `slaves[].sdoInit`, plus raw `canopen.data` / `canopen.up` preservation when those files exist
