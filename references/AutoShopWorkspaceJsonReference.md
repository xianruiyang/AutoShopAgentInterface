...
- CANopen: dynamic `配置/CAN(CANopen)/_node.config.json`, read-only `canOpen.catalog`, writable sampled `canOpen.dataConfig.network` fields, existing `objectTable[]` values, template-based `slaves[]` add/delete with optional `templateNodeId`, `slaves[].general.producerHeartbeatTimeMs`, existing `rxPdos[]`/`txPdos[]` summary values, existing `slaves[].sdoInit[]` value fields, existing `ioMappings[].startRegister` D start registers parsed from `canopen.data`, plus raw `canopen.data` / `canopen.up` preservation when those files exist
