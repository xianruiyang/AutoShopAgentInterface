...
适用版本：`autoshop-agent.exe v0.8.154`。
...
| `canLink.programConfig` | 从 `CANLink.prg` 解析出的 CANLink3.0 KLC record 视图，包含 header、CRC16/MODBUS 校验、网络记录、从站记录、发送配置、接收许可表、同步写 alias、只读同步视图和原始 records。 |
...
| `canLink.programConfig.receiveConfigurations[]` | CANLink3.0 “接收配置”许可表。当前支持接收许可条目的新增、删除和修改，每条最多 8 个 `allowedSenderStations`。 |
| `canLink.programConfig.syncMappings[]` | AutoShop “同步写”页既有行的可写 alias，底层仍是同一批 `sendConfigurations[]`。保持导出的行数和 `sourceRecordId/sourceEntryIndex`，可改触发方式、触发 M/周期、发送/接收站号、D/H 寄存器和 `registerCount`；与 `sendConfigurations[]` 同时编辑时必须语义一致。 |
| `canLink.programConfig.syncView[]` | AutoShop “同步写”页的派生只读回显，底层来自 `sendConfigurations[]`；该字段只读，直接修改会被拒绝。需要按同步写语义修改既有行时优先改 `syncMappings[]`。 |
      ...
      "syncMappings": [
        {
          ...
          "section": "host-send",
          "triggerMode": "event-m",
          "triggerCondition": "M120",
          "senderStation": 63,
          "senderRegister": "D200",
          "receiverStation": 1,
          "receiverRegister": "H1112",
          "registerCount": 2
        }
      ]
    }
...
当 `canLink.portConfig.parameters.protocol` 为 `CANOpen` 时，CAN 配置节点会按 AutoShop 工程树导出为 `配置/CAN(CANopen)/_node.config.json`，并附加 `canOpen` 诊断/窄范围写回对象；CANLink 协议仍导出为 `配置/CAN(CANLink)/_node.config.json`。
...
| `canOpen.catalog.deviceLibraryRoot` / `deviceLibraryIndex` | 当前命中的外部设备模板库根目录和 CANopen 索引文件。配置 `deviceLibraryPath` 或环境变量 `AUTOSHOP_AGENT_DEVICE_LIBRARY` 后，CLI 会读取 `<root>/canopen/index.json` 或直接读取 `canopen/index.json` 所在目录。 |
| `canOpen.catalog.devices[]` | 从 AutoShop EDS 文件和 CANopen 设备模板库合并出的设备列表，包含 `toolboxName/model/vendorNumber/productNumber/revisionNumber/productName/sourceRelative`、支持波特率、RxPDO/TxPDO 数量、对象字典摘要、`templateAvailable`、`templateNodeIds`、`libraryTemplateId`、`libraryPath`、`libraryVerified` 和 `librarySha256`。 |
...
| `canOpen.dataConfig.slaves[]` | 从真实 `canopen.data` 回读的节点，包含 `nodeId`、可选新增模板 `templateNodeId`、`toolboxName`、匹配到的 EDS/模板库 `sourceRelative/catalogKey`、identity、`general`、`rxPdos[]`、`txPdos[]`、`sdoInit[]`、`ioMappings[]` alias 和按 AutoShop 页签组织的 `pages[]`。既有 General 生产心跳时间、PDO 摘要、服务数据对象页行值和 I/O Mapping D 起始地址可写；也支持基于当前工程已有模板节点或 `deviceLibraryPath/canopen` 真实模板新增/删除从站，写回会重建 record 2/3/4/5、同步 objectTable/record 5 和 CRC。 |
| `canOpen.dataConfig.slaves[].pages[]` | CANopen 从站设置页签式入口，当前导出 `slaveNode`/从站节点、`receivePdo`/接收PDO、`sendPdo`/发送PDO、`sdoInit`/服务数据对象、`ioMapping`/I/O映射、`debug`/调试、`deviceInfo`/设备信息。可写字段可直接改 `fields[].value`，apply 会投影到现有安全语义字段；`editable=false` 的字段会拒绝写回并提示推荐路径。调试页、设备信息页和从站节点页的专家设置复选框当前没有离线可写样本，按只读诊断导出。 |
...
| `canOpen.jsonCreateSupported` / `canOpen.dataConfig.jsonWriteSupported` | `jsonCreateSupported=false`；存在 `canopen.data` 时 `dataConfig.jsonWriteSupported=true`，允许修改 `network` 中已采样主站字段、既有 objectTable 值、`slaves[].general.producerHeartbeatTimeMs`、既有 RxPDO/TxPDO 摘要、既有 `sdoInit[]` 行值、既有 I/O Mapping D 起始地址，或通过 `slaves[]` 基于当前工程模板节点/外部 CANopen 模板库增删从站并重算 CRC。`semanticWriteSupport` 会列出主站 record0、顶层和从站 alias 的可写入口。仍不允许从 EDS/catalog 在无真实模板时伪造 CANopen 从站，也不允许新增/删除 PDO、SDO 行或 I/O Mapping。 |

`canOpen.catalog` 解决 EDS 目录可见性、对象字典核对和外部 CANopen 模板发现；它不是纯 EDS 生成入口。`canOpen.dataConfig.network` 已支持主站 record0 中已采样字段窄范围写回；例如可把心跳生产时间、SDO 超时、从站接收/发送自动映射 D 起始和站点监控 D 起始写入 record0 并重算 CRC，已用 `[1]IS620_V056` 样本验证 export/edit/apply/re-export 和 AutoShop 主站设置页直接读回。`canOpen.dataConfig.objectTable[]` 已支持既有对象值窄范围写回；例如 `0x1017:0` 心跳生产时间会同步 record 2 对象表、record 3 运行镜像和启用标志，并重算 CRC。`canOpen.dataConfig.slaves[].general.producerHeartbeatTimeMs` 是同一字段的从站 General 页 alias，已用 `[1]IS620_V056` 样本验证 export/edit/apply/re-export；也可通过 `slaves[].pages[]` 中 `slaveNode.producerHeartbeatTimeMs` 修改。`canOpen.dataConfig.slaves[]` 支持基于当前工程已有模板节点或外部 CANopen 模板库增删从站；新增对象可填写 `nodeId` 和 `templateNodeId`，也可填写 `toolboxName` / `catalogKey` 命中右侧设备库模板；单模板工程省略 `templateNodeId` 时使用现有从站作为模板。写回会克隆并重写 record 2/3/4/5 的节点号、对象表、PDO/I/O 映射块和 CRC；当前工程模板路径已用 AutoShop UI 新增/删除 IS620 样本做字节级匹配验证，外部模板库路径已用真实 workspace 中的 CANopen 模板 JSON dry-run/apply/re-export 验证新增 node 3、D 映射重排和 CRC 有效。`canOpen.dataConfig.slaves[].rxPdos[]` / `txPdos[]` 是 objectTable 的 PDO 语义视图，可修改既有 PDO 的 `enabled`、`canId`、`cobId`、`transmissionType`、`eventTimeMs` 和现有映射对象值；`canId` 对应 AutoShop PDO 属性页显示的低 29 位 CAN-ID，写回会保留 raw `cobId` 高位控制位，`cobId` 仍是完整通信对象值的高级入口；PDO 数量或映射数量变化会被拒绝。`canOpen.dataConfig.slaves[].sdoInit[]` 是 AutoShop `服务数据对象` 页的既有行 alias，可修改既有行 `valueUnsigned`、`dataHex` 或 `rawValueHex`，并同步到底层 objectTable；行数变化和下载列标记变化会被拒绝。`canOpen.dataConfig.ioMappings[]` 和 `slaves[].ioMappings[]` 指向同一 record 5 映射，已在 `[1]IS620_V056` 样本上验证把 `D7000` 改为 `D7010` 后 re-export 回读 `D7010...D7013`、从站 alias 同步、CRC 有效；如果顶层和从站 alias 对同一映射写入不同 D 起始地址会被拒绝。当前 `workspace apply` 仍会拒绝直接添加顶层 `canOpen.slaves`，也不会在没有当前工程模板或外部 `canopen` 真实模板时根据 catalog/EDS 伪造 `CANopen` 配置文件。
...
AutoShop 4.10 保存从站后会生成 `canopen.data`；当前样本确认其为 `NOC` header + `E3/E4` records + 大端 `CRC16/MODBUS` 尾校验。CLI 已将 `canopen.data` / `canopen.up` 注册到 CAN 配置节点；若工程中实际存在，`workspace export` 会把它们作为 `files[]` 原始成员导出，`workspace apply` 可按 `contentHex` / `contentBase64` 保真回写。`deviceLibraryPath/canopen/index.json` 中的每个模板必须引用 AutoShop 实际保存的 `canopen.data` 模板，索引至少应提供 `templateId`、`toolboxName` 或 `catalogKey` 以及 `sourceRelative/path`；模板 JSON 可通过 `instanceTemplate.canopenDataHex` / `canopenDataBase64` 或 raw files 携带完整 `canopen.data`。未采样的主站 record0 余留字节、无当前工程/模板库真实模板的纯 EDS 从站生成、非空 SDO 初始化新增删除、I/O Mapping 新增删除、General 其它字段与 PDO 新增删除仍不能手写生成；已采样验证的主站 record0 字段、既有 objectTable 值、General 生产心跳 alias、既有 PDO 摘要字段、既有 I/O Mapping D 起始地址和基于当前工程/外部模板库模板的从站增删可通过 `canOpen.dataConfig` 修改。
