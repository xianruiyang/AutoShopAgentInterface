...
```text
编程                         -> folder.txt, 已发现的程序块/FB/FC 文件
编程/程序块                  -> folder.txt, folder.txt 中 PROG 段或工程根目录 *.ST
编程/程序块/MAIN             -> MAIN.ST
编程/程序块/SBR_001          -> SBR_001.ST
编程/程序块/INT_001          -> INT_001.ST
编程/功能块(FB)              -> folder.txt, folder.txt 中 FB 段登记的文件
编程/函数(FC)                -> folder.txt, folder.txt 中 FC 段登记的文件
配置/输入滤波             -> Config.sdt
...
编程节点也会出现在 `project node list --category program` 中。`.ST` 程序节点支持文件级导出/导入和 `project node refresh --name MAIN`；文本级查看、导出和写回仍建议使用 `pou show/export/import`，因为它会只替换 `.ST` 容器中的 LiteST 文本块。
