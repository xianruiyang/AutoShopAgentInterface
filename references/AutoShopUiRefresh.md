...
3. 优先复用刷新前记录的同一个 AutoShop 进程，通过“打开工程”对话框重新打开工程文件；只有原进程不存在时才使用 `autoShopExePath` 或文件关联启动工程文件。
4. 按刷新前记录的窗口列表重新打开对应工程树节点，最后把焦点切回刷新前的活动窗口。

该流程会短暂切前台，并使用剪贴板粘贴工程路径。若 AutoShop 弹出保存提示或关闭工程失败，命令会停止并报错；先处理未保存内容后再执行。
