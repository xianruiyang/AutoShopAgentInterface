...
- EtherNet/IP 节点若能识别 `EIP.dat`，会额外导出 `ethernetIP`：`devices` 是工程树下的 EIP_Card/H5U/Easy 等设备/从站，可通过顶层字段、`parameters`、`records[].value` 修改，通过删除数组项删除，写成空数组 `[]` 会清空 EtherNet/IP 从站设备表，通过 `toolboxName`、`templateKey`、高级字段 `catalogKey` 或完整 `records` 追加；`toolboxName` 写 AutoShop 工具箱名称，`catalogKey` 来自 `ethernetIP.catalog.devices[].key` 且仅用于高级诊断，同型号 `templateAvailable=true` 时会克隆完整模板，并从 catalog 默认补齐 `vendorId`、`productType`、`productCode`、`majorRevision`、`minorRevision` 等 identity 字段；当前工程无同型号模板但 `deviceLibraryPath` 或 `AUTOSHOP_AGENT_DEVICE_LIBRARY` 指向的设备库存在语义模板时，会从库中克隆完整 `records`。仍无语义模板时默认拒绝，以保证 AutoShop UI 参数保真。确实只需要基础 EDS 设备记录时，必须显式设置 `allowGeneratedFromCatalog=true`；`Generic_EtherNet_IP_device` 当前只有 raw 文件保真模板，不能通过 `ethernetIP.devices[]` 字段级新增。需要 100% 保留厂家私有页面隐藏字段时，仍应优先使用同型号模板或从其他导出映射复制完整记录。`producerTags` 是生产者标签，`serverMessageTags` 是服务消息标签，`adapter.connections` 是 Adapter 连接列表；每条连接下的 `outputDatasets`/`inputDatasets` 映射 I/O 数据集。Adapter I/O 数据集的 `dataType` 只能使用 AutoShop 下拉实际支持的 `INT|DINT|REAL`，导出 JSON 会在 `availableAdapterDataTypes` 中列出；`BOOL`、`BYTE`、`STRING` 等只属于标签层通用类型，不能写入 Adapter 数据集。`workspace apply` 会重建 `EIP.dat`、同步 `EIP.datBAK` 并重算尾部 CRC32；`EIP.data`、`SYS_EIP.eIPgvt` 仍作为真实成员文件保留在 `files` 中。
