...
- 对共享 EtherCAT 私有文件的节点，`workspace export` 会在 `EtherCat.dat`、`EtherCat.tmp`、`EtherCat.datBAK` 中选择内容最完整的镜像作为 `ethercat`、`motionAxis`、`axisGroup` 的 `sourceRelative`；这用于处理 AutoShop 把完整状态保留在 BAK、主 dat/tmp 只剩主站记录的情况。应用仍同步写回三个镜像。
