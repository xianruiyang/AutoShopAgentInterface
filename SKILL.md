...
- 当目标工程当前已在 AutoShop 打开时，默认必须先通过 `ui windows --format json` 确认主窗口 `title` 中的工程路径就是目标工程，再执行 `ui close-project --project <dir> --state D:\program\PLC\AutoShopAgentInterfaceWork\current-project-state.json --format json` 记录并关闭工程；完成 workspace 编辑和 `workspace apply` 后，再执行 `ui restore-project --state D:\program\PLC\AutoShopAgentInterfaceWork\current-project-state.json --format json` 恢复工程和窗口。若 AutoShop 未打开目标工程则跳过关闭/恢复；若打开的是其他工程，不要关闭。不要把 `--allow-open-project` 作为常规写回路径；只有用户明确要求跳过关闭/恢复流程时才可作为例外使用，并要说明风险。
...
- `ui compile`、`ui compile-all`、`ui run`、`ui stop`、`ui monitor` 分别对应 AutoShop 工具栏/快捷键 `Ctrl+F7`、`F7`、`F5`、`F6`、`F3`，默认通过主窗口 `WM_COMMAND` 静默触发，并返回底部“信息输出窗口”输出及本次新弹出的 AutoShop 对话框文本；`ui monitor/run/stop` 默认会读取并立即确认连接状态类 `确定` 弹窗，避免弹窗堆积。`ui output` 可单独读取 `compile|communication|transfer|search|visible|all` 输出页。输出默认 `--lines errors` 只返回错误行；完整日志使用 `--lines all --tail 0`。
...
.\scripts\autoshop-agent.exe workspace apply --project D:\program\PLC\project001 --in D:\program\PLC\AutoShopAgentInterfaceWork\current-export --format json
...
```powershell
.\scripts\autoshop-agent.exe ui close-project --project D:\program\PLC\project001 --state D:\program\PLC\AutoShopAgentInterfaceWork\current-project-state.json --format json
.\scripts\autoshop-agent.exe pou import --project D:\program\PLC\project001 --name MAIN --in D:\tmp\MAIN.st.txt --format json
.\scripts\autoshop-agent.exe ui restore-project --state D:\program\PLC\AutoShopAgentInterfaceWork\current-project-state.json --format json
```
