...
- 使用 `templateKey` 或命中同型号模板的 `catalogKey` 新增 EtherCAT 从站时，CLI 会克隆完整私有从站段，并自动重写拓扑字段和段内 `_IQ/_Q` 站号引用。`deviceName` 未填写时会自动生成 AutoShop 可识别的数字副本名，例如已有 `SV820N` 和 `SV820N_1` 时新增为 `SV820N_2`；手工填写时必须保持 ESI 型号名或数字副本后缀，`JSON_xxx` 这类任意名称会被 `workspace apply` 拒绝，避免 AutoShop 找不到 XML。
