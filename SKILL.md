...
- 使用 `templateKey` 新增 EtherCAT 从站时，CLI 会克隆完整私有从站段，并自动重写拓扑字段和段内 `_IQ/_Q` 站号引用。`name` / `parameters.deviceName` 必须保持 AutoShop/ESI 可识别的型号名或同型号实例名，例如 `GL20-RTU-ECT_3`；不要填 `JSON_xxx` 这类任意名称，否则 AutoShop 可能找不到 XML 或显示乱码节点。
