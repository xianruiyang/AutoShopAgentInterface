...
- EtherNet/IP 节点若能识别 `EIP.dat`，会额外导出 `ethernetIP`：`devices` 是工程树下的 EIP_Card/H5U/Easy/Generic 等设备/从站，可通过顶层字段、`parameters`、`records[].value` 和 `privateRecords[].value` 修改，通过删除数组项删除，写成空数组 `[]` 会清空 EtherNet/IP 从站设备表，通过 `toolboxName`、`templateKey`、高级字段 `catalogKey` 或完整 `records/privateRecords` 追加；`toolboxName` 写 AutoShop 工具箱名称，`catalogKey` 来自 `ethernetIP.catalog.devices[].key` 且仅用于高级诊断，同型号 `templateAvailable=true` 时会克隆完整模板，并从 catalog 默认补齐 `vendorId`、`productType`、`productCode`、`majorRevision`、`minorRevision` 等 identity 字段；当前工程无同型号模板但 `deviceLibraryPath` 或 `AUTOSHOP_AGENT_DEVICE_LIBRARY` 指向的设备库存在语义模板时，会从库中克隆完整 `records + privateRecords`。仍无语义模板时默认拒绝，以保证 AutoShop UI 参数保真。确实只需要基础 EDS 设备记录时，必须显式设置 `allowGeneratedFromCatalog=true`；`Generic_EtherNet_IP_device` 当前作为 private-only 模板接入，不生成 primary device records。`producerTags` 是生产者标签，`serverMessageTags` 是服务消息标签，`adapter.connections` 是 Adapter 连接列表；每条连接下的 `outputDatasets`/`inputDatasets` 映射 I/O 数据集。Adapter I/O 数据集的 `dataType` 只能使用 AutoShop 下拉实际支持的 `INT|DINT|REAL`，导出 JSON 会在 `availableAdapterDataTypes` 中列出；`BOOL`、`BYTE`、`STRING` 等只属于标签层通用类型，不能写入 Adapter 数据集。
- `workspace apply` 会重建 `EIP.dat`、同步 `EIP.datBAK` 并重算尾部 CRC32；EtherNet/IP 设备增删改还会同步 `SYS_EIP.eIPgvt`：`EIP_Card` 补齐 `_IPn_2` 到 `_IPn_53`，`Easy`/`H5U` 等普通 primary 设备补齐 `_IPn_2`/`_IPn_3`，并联动设备序号、私有设备类型码、IP、primary 关联字段和 I/O 变量名。已确认的私有设备类型码为 `EIP_Card=0`、`Easy=1`、`H5U=2`、`Generic_EtherNet_IP_device=3`；Generic 作为 private-only 设备会写入 AutoShop 样本确认的工程树码 `7`。导出 JSON 无语义变化时，CLI 不会为了规范化私有字节而重写 `EIP.dat`，但仍会检查 `EIP.datBAK` 等镜像是否漂移，漂移时同步到当前 `EIP.dat` 字节；`EIP.data` 仍作为真实成员文件保留在 `files` 中。
- 对共享 EtherCAT 私有文件的节点，`workspace export` 优先使用 AutoShop 当前主文件 `EtherCat.dat` / `EtherCat.tmp` 生成 `ethercat`、`motionAxis`、`axisGroup` 语义 JSON；只有两个主文件都无法读取或解析时才回退 `EtherCat.datBAK`，避免 AutoShop 手动保存后留下的旧 BAK 被误认为当前状态。应用仍同步写回三个镜像。
...
- 外部写回后，AutoShop 已打开编辑窗口不会自动刷新；ST/普通树节点可用 `workspace apply --refresh`、`ui refresh --program <name>`、`ui refresh-path --path <tree-path>` 或旧别名 `refresh --program <name>` 关闭并重新打开对应窗口。变量表等工程级缓存优先用 `ui close-project` 记录当前工程、已打开窗口和活动窗口并关闭工程，再用 `ui restore-project` 读取状态文件、优先复用同一个 AutoShop 进程打开工程、恢复窗口和焦点；`ui refresh-project` 只保留为一次性兼容封装。`restore-project` 会短暂切前台并使用剪贴板粘贴工程路径。
